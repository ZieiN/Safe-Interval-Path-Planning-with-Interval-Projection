...
//set<int> rsrv_tbl_edges[MXH][MXW];
        ...
        if(v==0) {
            for (auto a: actions[o][v]) {
                ...
                if (new_tlower > new_tupper)
                    continue;
                it = rsrv_tbl[new_x][new_y].lower_bound({new_tlower, INF});
                auto it_prev = it;
                --it_prev;
                while (it_prev->second < new_tupper) {
                    ...
                    if (new_tlower1 <= new_tupper1)
                        OPEN.insert(Bot(new_x, new_y, new_o, new_v, new_tlower1, it->first-1));
                    ++it;
                    ++it_prev;
                }
            }
            ...
            for(auto a:actions[o][v]){
                ...
                if(new_tlower >= safe_interval_upper_bound)
                    continue;
                it = rsrv_tbl[new_x][new_y].lower_bound({new_tlower, INF});
                auto it_prev = it;
                --it_prev;
                if((new_tlower-a.cost > (it_prev->second)) && (new_tlower<(it->first))){
                    OPEN.insert(Bot(new_x, new_y, new_o, new_v, new_tlower, it->first-1));
                }
            }
        }
    }
